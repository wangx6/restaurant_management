...
demoApp.controller('SimpleController', function ($scope, simpleFactory, $location,$http,$timeout){
    ...
    $scope.initTables = function(restaurantSetup){
        ...
        {
            var noOfTables = restaurantSetup.noOfTables;
    ...
    $scope.$watch('tables',function(){
        ...
        /*$.post('process.basket.php',$('#process_quanlity').serialize(),function(data){
            var str=data;
            var data=str.split(":||:");

            $('.warning_cover').html('( '+data[0]+' )');
            $('.basket_numberofitems').html('( '+data[0]+' )');
            $('#added_quanlity').html('( '+data[1]+' )');
            $('#added_prd_description').html(data[2]);
            $('.total_price').html('&#8364; '+data[3])

            $('#add_to_basket_notice_blk').fadeIn(1000,function(){
                setTimeout(function(){
                    $('#add_to_basket_notice_blk').fadeOut(1000,function(){
                        $('#added_quanlity').html('');
                        $('#added_prd_description').html('');
                    });
                },800);
            });
        });*/
...
demoApp.directive('testd',function(){
    var linkfn;
...
angular.module('app.directives.contactCard',[])
    .directive('contactCard',function(){
        return{
            ...
            template:"<h3>{{first}} {{last}} has a friend {{friend}}</h3>"
        }
    })
